...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.updateTable('profiles').set({ subscription_status: 'hard_locked' }).where('id', '=', id).execute();
    ...
    return NextResponse.json({ success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to suspend user' } }, { status: 500 });
  }
}
