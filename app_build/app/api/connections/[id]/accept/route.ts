...
export async function PATCH(
  ...
  try {
    ...
    const connection = await connectionService.acceptConnection(id, user.id);
    ...
    if (message === 'HANDSHAKE_ACCEPT_NOT_TARGET') {
      return NextResponse.json(
        { success: false, error: { code: 'HANDSHAKE_ACCEPT_NOT_TARGET', message: 'Only the target can accept this handshake' } },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: { code: 'INTERNAL_ERROR', message: 'Failed to accept connection' } },
      { status: 500 }
    );
  }
}
